...
/**
 * Define a message to the ambit msg  
 *
 */
public class AmbitMsgHeader {
	 ...
	 * Message with data
	 */
	public AmbitMsgHeader(AmbitCommand command, byte[] msgData) {
		...
		data[0] = REPORT_ID;   // report id (63)
		data[1] = (byte) (packetPayloadLen + 12 + 8);  // UL
		data[2] = 0x5D;        // MP 5d : premier, 5e suivants
		...
		data[15] = seq[1];       // seq  
		/*data[16] = 0x00;         // payload len 
		data[17] = 0x00;         // payload len
		data[18] = 0x00;         // payload len  
		data[19] = 0x00;         // payload len 
		*/
		...
		//int crcMsg = CRCUtil.crc_16_ccitt_init(Arrays.copyOfRange(data, 8, 20), 12, crc);
